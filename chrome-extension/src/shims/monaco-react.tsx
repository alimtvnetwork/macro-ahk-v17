...
/* ------------------------------------------------------------------ */
/*  DiffEditor fallback — side-by-side textareas                      */
/* ------------------------------------------------------------------ */
...
export function DiffEditor({ height = "300px", original = "", modified = "" }: DiffEditorProps) {
  const resolvedHeight = typeof height === "number" ? `${height}px` : height;
  return (
    <div className="flex gap-1 w-full" style={{ height: resolvedHeight }}>
      <textarea
        readOnly
        value={original}
        className="flex-1 p-2 font-mono text-xs bg-muted text-foreground border rounded resize-none"
