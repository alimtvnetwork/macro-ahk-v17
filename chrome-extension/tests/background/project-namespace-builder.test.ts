...
describe("buildProjectNamespaceScript", () => {
    ...
    it("handles missing window.marco with deferred proxy warning", () => {
        const result = buildProjectNamespaceScript(DEFAULT_CTX);
        expect(result).toContain("deferred proxy");
    });

    it("includes all sub-namespaces", () => {
        const result = buildProjectNamespaceScript(DEFAULT_CTX);
        for (const ns of ["vars", "urls", "xpath", "cookies", "kv", "files", "meta", "log", "scripts", "db", "api", "notify", "docs"]) {
    ...
    it("includes notify sub-namespace with toast, dismiss, and error methods", () => {
        const result = buildProjectNamespaceScript(DEFAULT_CTX);
        expect(result).toContain("notify: Object.freeze({");
        expect(result).toContain("toast: function(msg, level, opts)");
        expect(result).toContain("dismiss: function(id)");
        expect(result).toContain("dismissAll: function()");
        expect(result).toContain("onError: function(cb)");
        expect(result).toContain("getRecentErrors: function()");
    });
});
