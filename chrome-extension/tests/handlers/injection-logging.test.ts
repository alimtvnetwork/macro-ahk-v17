...
} = await import("../../src/background/injection-diagnostics");
...
describe("Injection Logging — shared diagnostics helper", () => {
    beforeEach(async () => {
        resetMockStorage();
        await setupDbs();
    });

    it("persists warning diagnostics into Logs", async () => {
        await persistInjectionWarn(
            "SCRIPT_SKIPPED_MISSING",
            "[injection:resolve] Script not found in store: macro-looping.js",
            { scriptId: "macro-looping.js", projectId: "proj-1" },
        );

        const logs = queryLogs();

        expect(logs).toHaveLength(1);
        expect(logs[0].action).toBe("SCRIPT_SKIPPED_MISSING");
        expect(logs[0].category).toBe("INJECTION");
        expect(logs[0].ScriptId).toBe("macro-looping.js");
        expect(logs[0].ProjectId).toBe("proj-1");
    });

    it("persists error diagnostics into Errors", async () => {
        await persistInjectionError(
            "SPA_REINJECT_FAILED",
            "[spa-reinject] Re-inject failed for tab 9: CSP blocked",
            { scriptId: "macro-looping.js", projectId: "proj-2", contextDetail: "tabId=9" },
        );

        const errors = queryErrors();

        expect(errors).toHaveLength(1);
        expect(errors[0].ErrorCode).toBe("SPA_REINJECT_FAILED");
        expect(errors[0].category).toBe("INJECTION");
        expect(errors[0].ScriptId).toBe("macro-looping.js");
        expect(errors[0].ProjectId).toBe("proj-2");
        expect(errors[0].context).toBe("tabId=9");
    });
});
