 ...
 * instruction.json is the sole source of truth.
 */
/* eslint-disable sonarjs/no-duplicate-string -- filesystem ops with branching */
function copyProjectScripts(): Plugin {
  return {
    ...
    writeBundle() {
      ...
      // Generate seed-manifest.json from compiled instruction.json files
      try {
        const rootDir = resolve(__dirname, '..');
        const outPath = resolve(buildDistDir, 'projects', 'seed-manifest.json');
        execSync(
          `node scripts/generate-seed-manifest.mjs --out "${outPath}"`,
