# Automator v6.54 - Technical Specification
...
| Module | Purpose |
|--------|---------|
| Config.ahk | Read all config.ini values (including JS executor IDs, confirm delay) |
...
InjectViaDevTools(js) {
    ...
    ; Subsequent:  F12 (close) -> Sleep(300) -> Ctrl+Shift+J (reopen) -> wait consoleOpenDelayMs
    ;              This close-reopen strategy guarantees console input focus
...
**Key**: DevTools opens ONCE via `Ctrl+Shift+J`. Subsequent injections use a
**close-and-reopen** strategy (`F12` close → `Ctrl+Shift+J` reopen) to guarantee
console input focus. This replaces the previous `Escape`-based focus which was unreliable.

### Console Focus Management (v6.2 Change)

**Before v6.2**: Subsequent injections sent `Escape` to dismiss autocomplete and refocus
the console input. This was unreliable — DevTools focus could shift to other panels,
autocomplete popups, or the console output area, causing `Ctrl+V` to paste into nothing.

**After v6.2**: The close-reopen strategy ensures the console input is always focused:
1. `F12` closes DevTools entirely
2. `Sleep(300)` waits for close
3. `Ctrl+Shift+J` reopens DevTools with Console panel focused
4. `Sleep(consoleOpenDelayMs)` waits for console to be ready

**Trade-off**: Adds ~1 second per injection but eliminates all focus-related failures.
...
ComboRetryCount=2
ComboRetryDelayMs=1000
ComboConfirmDelayMs=500          ; Delay after option select before clicking Confirm
...
RunCombo(direction)
    ...
    |     YES -> F6 (focus console) -> paste window.__comboSwitch(direction), DONE
    |     NO  -> continue to full injection
    |
    +-- FULL PATH: Load combo.js
          +-- Replace ALL placeholders
          +-- InjectJS(templated combo.js)
          +-- Set scriptEmbedded := true
                +-- Embeds UI + runs combo
```
...
1. Check `document.getElementById(SCRIPT_ID)` -> if exists, skip UI creation
2. Define helper functions: findElement (5-method fallback), pollForElement, logging
3. Define `runComboSwitch(direction)` with 8 steps
4. Define `executeJsFromTextbox()` for JS executor
5. Define `updateStatusDisplay()` for live status
6. Define `addHistoryEntry()` + `renderHistory()` for action log
7. Define `flashStatus()` for success animation
8. Define `createControllerUI()` for full panel
9. Expose `window.__comboSwitch = runComboSwitch`
10. Expose `window.__executeJs = executeJsFromTextbox`
11. Create hidden marker div with SCRIPT_ID
12. Call `createControllerUI()` to add UI elements
13. Install MutationObserver for SPA persistence
...
+---------------------------------------------+
| [Up]  [Down]  [Status]                       |  <- Buttons (highlight during combo)
| Ctrl+Alt+Up / Down / Ctrl+Alt+S              |  <- Hint
|                                              |
| Credits: 36.6/100 | Daily: 5/5 | Free: Yes  |  <- Credit status display
| P01: 36.6/100  P02: 0/100                   |  <- Per-workspace breakdown
| Last checked: 14:32:05 (api)                 |
|                                              |
| NOW: ProjectA | ^ ProjectC . v ProjectB      |  <- Status display
|                                              |
| JS Executor (Ctrl+/ to focus, Ctrl+Enter)    |  <- Label
| +----------------------------------+ [Run]   |
| | Enter JavaScript code here...    |         |  <- Textbox + Execute
| +----------------------------------+         |
|                                              |
| Recent Actions                               |  <- Combo history
| | 05:15:30 PM  v  ProjectB        |         |
| | 05:14:22 PM  ^  ProjectC        |         |
|                                              |
| JS Command History (click/Up/Down to recall) |  <- JS history
| | 14:32:05  ✓  document.title      |         |
| | 14:31:50  ✗  invalidFunc()       |         |
+---------------------------------------------+
```
...
EmbedMacroLoopScript()
    |-- Inject xpath-utils.js first (if present)
    |-- Read macro-looping.js from disk
    |-- Replace __PLACEHOLDER__ values from config.ini
    |-- InjectJS(templatedScript)      <-- shared function
