...
; Features:
;   - Smart Ctrl+Left/Right: ComboSwitch on settings, MacroLoop on project
...
#Include *i Includes\ExportCompiledJS.ahk
...
; ============================================
; Version auto-sync: AHK_BUILD_VERSION is authoritative
; If config.ini ScriptVersion differs, auto-update it silently
; ============================================
if (AHK_BUILD_VERSION != scriptVersion) {
    WarnLog("VERSION AUTO-SYNC: config.ini had v" scriptVersion ", updating to v" AHK_BUILD_VERSION)
    try {
        IniWrite(AHK_BUILD_VERSION, configFile, "General", "ScriptVersion")
        scriptVersion := AHK_BUILD_VERSION
        InfoLog("config.ini ScriptVersion auto-synced to v" AHK_BUILD_VERSION)
    } catch as err {
        WarnLog("Failed to auto-sync version: " err.Message " — continuing with mismatch")
    }
}
...
Tray.Add("=== EXPORT COMPILED JS ===", MenuDummy)
Tray.Disable("=== EXPORT COMPILED JS ===")

Tray.Add("Export combo.js", MenuExportCombo)
Tray.SetIcon("Export combo.js", ICON_DLL, ICON_EXPORT)

Tray.Add("Export macro-looping.js", MenuExportMacroLoop)
Tray.SetIcon("Export macro-looping.js", ICON_DLL, ICON_EXPORT)

Tray.Add("Export Full Bundle", MenuExportBundle)
Tray.SetIcon("Export Full Bundle", ICON_DLL, ICON_EXPORT)

Tray.Add()

Tray.Add("=== COPY JS (CLIPBOARD) ===", MenuDummy)
Tray.Disable("=== COPY JS (CLIPBOARD) ===")

Tray.Add("Copy combo.js", MenuCopyCombo)
Tray.SetIcon("Copy combo.js", ICON_DLL, ICON_EXPORT)

Tray.Add("Copy macro-looping.js", MenuCopyMacroLoop)
Tray.SetIcon("Copy macro-looping.js", ICON_DLL, ICON_EXPORT)

Tray.Add("Copy Full Bundle", MenuCopyBundle)
Tray.SetIcon("Copy Full Bundle", ICON_DLL, ICON_EXPORT)
...
; ============================================
; Tray menu handlers - Export Compiled JS
; ============================================
MenuExportCombo(*) {
    InfoLog("Tray menu: Export combo.js clicked")
    ExportComboJS()
}
MenuExportMacroLoop(*) {
    InfoLog("Tray menu: Export macro-looping.js clicked")
    ExportMacroLoopJS()
}
MenuExportBundle(*) {
    InfoLog("Tray menu: Export Full Bundle clicked")
    ExportBundledJS()
}

; ============================================
; Tray menu handlers - Copy JS (clipboard only, no file)
; ============================================
MenuCopyCombo(*) {
    InfoLog("Tray menu: Copy combo.js clicked")
    try {
        compiledJs := BuildComboJS("down")
        A_Clipboard := compiledJs
        TrayTip("combo.js copied to clipboard (" StrLen(compiledJs) " chars)", "Automator", 1)
    } catch as err {
        ErrorLog("MenuCopyCombo FAILED: " err.Message)
        TrayTip("Copy combo.js failed: " err.Message, "Automator Error", 3)
    }
}
MenuCopyMacroLoop(*) {
    InfoLog("Tray menu: Copy macro-looping.js clicked")
    try {
        compiledJs := BuildMacroLoopJS()
        A_Clipboard := compiledJs
        TrayTip("macro-looping.js copied to clipboard (" StrLen(compiledJs) " chars)", "Automator", 1)
    } catch as err {
        ErrorLog("MenuCopyMacroLoop FAILED: " err.Message)
        TrayTip("Copy macro-looping.js failed: " err.Message, "Automator Error", 3)
    }
}
MenuCopyBundle(*) {
    InfoLog("Tray menu: Copy Bundle clicked")
    try {
        xpathUtilsFile := A_ScriptDir "\xpath-utils.js"
        xpathUtilsJs := ""
        if FileExist(xpathUtilsFile) {
            xpathUtilsJs := FileRead(xpathUtilsFile, "UTF-8")
        }
        macroLoopJs := BuildMacroLoopJS()
        bundle := ""
        if (StrLen(xpathUtilsJs) > 0) {
            bundle .= "// === PART 1: xpath-utils.js ===`n" . xpathUtilsJs . "`n`n"
        }
        bundle .= "// === PART 2: macro-looping.js ===`n" . macroLoopJs
        A_Clipboard := bundle
        TrayTip("Bundle copied to clipboard (" StrLen(bundle) " chars)", "Automator", 1)
    } catch as err {
        ErrorLog("MenuCopyBundle FAILED: " err.Message)
        TrayTip("Copy bundle failed: " err.Message, "Automator Error", 3)
    }
}
...
MenuHelpReadme(*) {
    readmePath := A_ScriptDir "\specs\README.md"
    isReadmePresent := FileExist(readmePath)
    if isReadmePresent {
        InfoLog("Opening specs\README.md")
        Run(readmePath)
    } else {
        WarnLog("README.md not found: " readmePath)
        MsgBox("README.md not found in specs folder.", "Automator v" scriptVersion)
    }
}

MenuHelpSpec(*) {
    specPath := A_ScriptDir "\specs\SPEC.md"
    isSpecPresent := FileExist(specPath)
    if isSpecPresent {
        InfoLog("Opening specs\SPEC.md")
        Run(specPath)
    } else {
        WarnLog("SPEC.md not found: " specPath)
        MsgBox("SPEC.md not found in specs folder.", "Automator v" scriptVersion)
    }
}
