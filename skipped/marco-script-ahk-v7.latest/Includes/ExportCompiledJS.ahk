...
SaveCompiledJS(scriptName, compiledJs, sourceFile := "") {
    ...
    header .= "// TIP: If Domain Guard blocks, run: window.__comboForceInject = true  first.`n"
    header .= "// ============================================`n`n"
...
; ============================================
; ExportBundledJS - Export bundle (xpath-utils + macro-looping) with all placeholders resolved
; Concatenates xpath-utils + macro-looping into a single self-contained JS file.
; NOTE: combo.js is NOT included — this is for standalone MacroLoop usage.
; ============================================
ExportBundledJS() {
    InfoLog("ExportBundledJS called from tray menu")
    try {
        ; Read xpath-utils.js (raw, no placeholders)
        xpathUtilsFile := A_ScriptDir "\xpath-utils.js"
        xpathUtilsJs := ""
        isXPathPresent := FileExist(xpathUtilsFile)
        if isXPathPresent {
            xpathUtilsJs := FileRead(xpathUtilsFile, "UTF-8")
        } else {
            WarnLog("ExportBundledJS: xpath-utils.js not found, skipping")
        }

        ; Build compiled macro-looping (no combo)
        ...
        isSuccess := SaveCompiledJS("bundle-all", bundle, "xpath-utils.js + macro-looping.js")
        if !isSuccess {
            ErrorLog("ExportBundledJS: SaveCompiledJS returned false")
        }
    } catch as err {
        ErrorLog("ExportBundledJS FAILED: " err.Message)
        TrayTip("Export bundled JS failed: " err.Message, "Automator Error", 3)
    }
}
