(function() {
  ...
  if (!isPageContext && !window.__comboForceInject) {
    console.warn(
      ...
      '  bypass: Set window.__comboForceInject = true before pasting to skip this check.\n' +
  ...
  function safeSetItem(key, value) {
    try {
      ...
      console.warn('[ComboSwitch] localStorage quota exceeded — scanning for bloated keys to purge');
      let purged = 0;
      for (let i = localStorage.length - 1; i >= 0; i--) {
        ...
        for (let p = 0; p < BLOATED_KEY_PATTERNS.length; p++) {
          if (k.indexOf(BLOATED_KEY_PATTERNS[p]) !== -1) {
            ...
            console.warn('[ComboSwitch] Purging bloated key: "' + k + '" (size=' + size + ')');
      ...
      if (purged > 0) {
        try {
          localStorage.setItem(key, value);
          console.log('[ComboSwitch] Retry succeeded after purging ' + purged + ' bloated key(s)');
          return true;
        } catch (e2) {
          console.error('[ComboSwitch] Retry failed even after purging — clearing all localStorage');
          localStorage.clear();
          try { localStorage.setItem(key, value); return true; } catch (e3) { return false; }
        }
      } else {
        console.error('[ComboSwitch] Quota exceeded but no bloated keys found — clearing all localStorage');
        localStorage.clear();
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (k && k.indexOf('ahk_bearer_') === 0 && k !== BEARER_STORAGE_KEY) {
          ...
          if (val) {
            safeSetItem(BEARER_STORAGE_KEY, val);
  ...
  function markBearerTokenExpired(controller) {
    ...
    var cookieBtnId = controller === 'combo' ? 'combo-quick-cookie-btn' : 'loop-quick-cookie-btn';
    ...
    logWarn('markBearerTokenExpired', '[' + controller + '] Bearer token marked as EXPIRED (401/403 received) — Paste & Cookie recovery buttons injected');
  }
  ...
  function getBearerTokenFromCookie() {
    ...
    try {
      ...
      logBanner(fn, '=== COOKIE DIAGNOSTIC START ===');
      logEntry(fn, 'document.cookie accessible: ' + (typeof document.cookie === 'string' ? 'YES' : 'NO'));
      logEntry(fn, 'Total cookies visible to JS: ' + cookieCount);
      logEntry(fn, 'Cookie names visible: [' + cookieNames.join(', ') + ']');
      logEntry(fn, 'Raw cookie string length: ' + rawCookie.length + ' chars');

      if (cookieCount === 0 || rawCookie.length === 0) {
        logWarn(fn, '⚠ NO cookies visible to JavaScript at all!');
        logWarn(fn, 'PROBABLE CAUSE: All cookies are HttpOnly (server-set, JS cannot read them)');
        logWarn(fn, 'HttpOnly cookies are visible in DevTools > Application > Cookies but NOT to document.cookie');
        logWarn(fn, 'SOLUTION: Copy the token manually from DevTools > Application > Cookies > lovable-session-id.id');
        logBanner(fn, '=== COOKIE DIAGNOSTIC END (no cookies) ===');
        return '';
      }
      ...
      for (var i = 0; i < cookies.length; i++) {
        ...
        if (c.indexOf('lovable-session-id.id=') === 0) {
          ...
          logEntry(fn, '✅ Found "lovable-session-id.id" cookie');
          logSub(fn, 'Value length: ' + (val ? val.length : 0));
          logSub(fn, 'Preview: ' + (val ? val.substring(0, 12) + '...REDACTED' : '(empty)'));
          if (val && val.length >= 10) {
            logInfo(fn, '✅ Cookie value is valid (len=' + val.length + ')');
            logBanner(fn, '=== COOKIE DIAGNOSTIC END (success) ===');
            return val;
          } else {
            logWarn(fn, '⚠ Cookie found but value too short (len=' + (val ? val.length : 0) + ', min=10)');
            logWarn(fn, 'PROBABLE CAUSE: Cookie was cleared/corrupted or session expired');
          }
        }
      }

      if (!hasTarget) {
        logWarn(fn, '❌ "lovable-session-id.id" NOT found among ' + cookieCount + ' visible cookies');
        logSub(fn, 'Visible cookie names: [' + cookieNames.join(', ') + ']');
        logWarn(fn, 'PROBABLE CAUSE: The session cookie is HttpOnly — JS cannot access it');
        logWarn(fn, 'HttpOnly cookies are set with "Set-Cookie: HttpOnly" flag by the server');
        logWarn(fn, 'They appear in DevTools > Application > Cookies but document.cookie cannot read them');
        logWarn(fn, 'SOLUTION: Manually copy from DevTools > Application > Cookies, or use Paste button');
      }
      logBanner(fn, '=== COOKIE DIAGNOSTIC END (not found) ===');
    } catch (e) {
      logError(fn, '❌ EXCEPTION reading cookies: ' + e.message);
  ...
  // config.ini > localStorage > lovable-session-id.id cookie
  function resolveToken() {
    var token = CREDIT_CFG.BEARER_TOKEN;
    if (token && token !== '__LOVABLE_BEARER_TOKEN__' && token !== '') return token;
    token = getBearerTokenFromStorage();
    if (token) return token;
    token = getBearerTokenFromCookie();
    if (token) {
      // Auto-save cookie token to localStorage for future use
      saveBearerTokenToStorage(token);
      logInfo('resolveToken', 'Recovered bearer token from session cookie → saved to localStorage');
    }
    return token;
  }
  ...
  function retrieveTokenFromCookie(controller) {
    ...
    logBanner(fn, '========== COOKIE TOKEN RETRIEVAL ==========');
    logEntry(fn, 'Step 1: Reading lovable-session-id.id cookie...');
    if (titleEl) { titleEl.textContent = 'Bearer Token 🍪 Reading cookie...'; titleEl.style.color = THEME.YELLOW; }

    var cookieVal = getBearerTokenFromCookie();
    if (!cookieVal || cookieVal.length < 10) {
      logWarn(fn, 'Step 1 FAILED: Cookie not found or too short (len=' + (cookieVal ? cookieVal.length : 0) + ')');
      logSub(fn, 'Available cookies: ' + (document.cookie ? document.cookie.split(';').map(function(c){ return c.trim().split('=')[0]; }).join(', ') : '(none)'));
      if (titleEl) { titleEl.textContent = 'Bearer Token 🔴 Cookie not found!'; titleEl.style.color = THEME.RED; }
      setTimeout(function() { if (titleEl) { titleEl.style.color = ''; } }, 3000);
      return;
    }

    logEntry(fn, 'Step 1 OK: Cookie found (len=' + cookieVal.length + ', preview=' + cookieVal.substring(0, 12) + '...REDACTED)');
    logEntry(fn, 'Step 2: Saving to localStorage...');
    ...
    logEntry(fn, 'Step 2 OK: Token saved to localStorage');
    logEntry(fn, 'Step 3: Verifying token via /user/workspaces API...');
    if (titleEl) { titleEl.textContent = 'Bearer Token 🔄 Verifying cookie token...'; titleEl.style.color = THEME.YELLOW; }

    var url = CREDIT_CFG.API_BASE + '/user/workspaces';
    var headers = { 'Accept': 'application/json', 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + cookieVal };
    logSub(fn, 'GET ' + url);
    logSub(fn, 'Headers: ' + JSON.stringify({ Authorization: 'Bearer ' + cookieVal.substring(0, 12) + '...REDACTED' }));

    fetch(url, { method: 'GET', headers: headers, credentials: 'include' })
      .then(function(resp) {
        logSub(fn, 'Step 3 Response: HTTP ' + resp.status + ' ' + resp.statusText);
        if (!resp.ok) {
          logWarn(fn, 'Step 3 FAILED: Token verification failed (HTTP ' + resp.status + ')');
          if (titleEl) { titleEl.textContent = 'Bearer Token 🔴 Cookie token INVALID (HTTP ' + resp.status + ')'; titleEl.style.color = THEME.RED; }
          setTimeout(function() { if (titleEl) { titleEl.style.color = ''; } }, 3000);
          return;
        }
        return resp.text().then(function(bodyText) {
          ...
          try { data = JSON.parse(bodyText); } catch(e) {
            logWarn(fn, 'Step 3: JSON parse failed — ' + e.message);
            return;
          }
          var wsCount = Array.isArray(data) ? data.length : '?';
          logEntry(fn, 'Step 3 OK: ✅ Token is VALID — ' + wsCount + ' workspaces returned');

          // Remove quick paste button if present
          var quickBtn = document.getElementById(pasteBtnId);
          if (quickBtn) quickBtn.remove();

          logEntry(fn, 'Step 4: Refreshing workspace data...');
          if (titleEl) { titleEl.textContent = 'Bearer Token 🔄 Refreshing data...'; titleEl.style.color = THEME.YELLOW; }

          parseApiResponse(data);
          autoDetectCurrentWorkspace(cookieVal, 'cookie-retrieve-' + Date.now()).then(function() {
            updateCreditDisplay();
            logEntry(fn, 'Step 4 OK: ✅ All data refreshed — workspace detected, credits updated');
            logBanner(fn, '========== COOKIE RETRIEVAL COMPLETE ==========');
            if (titleEl) { titleEl.textContent = 'Bearer Token ✅ Recovered from cookie (' + cookieVal.length + ' chars)'; titleEl.style.color = THEME.GREEN; }
            setTimeout(function() { if (titleEl) { titleEl.style.color = ''; } }, 4000);
          });
        });
      })
      .catch(function(err) {
  ...
  // v7.10.2: findAllByXPath — returns ALL matching nodes for multi-match validation
  function findAllByXPath(xpath) {
    const fn = 'findAllByXPath';
    if (!xpath || isEmptyString(xpath)) {
      logWarn(fn, 'XPath is empty, returning []');
      return [];
    }
    try {
      ...
      logSub(fn, 'XPath matched ' + nodes.length + ' node(s): ' + xpath);
      return nodes;
    } catch (e) {
      console.error(PREFIX + ' [' + fn + '] Invalid XPath: ' + xpath, e);
      return [];
  ...
  function saveKnownWorkspaces(labels) {
    ...
    try {
      ...
      if (added > 0) {
        safeSetItem(WS_SHARED_KEY, JSON.stringify(merged));
  ...
  function moveToWorkspace(targetWorkspaceId, targetWorkspaceName) {
    ...
    // v7.9.35: Unified token resolution (config > localStorage > cookie)
    let resolvedToken = resolveToken();
    if (!resolvedToken) {
        ...
        // Stacked credit bar: full bar = total capacity, colored = available
         + (function() {
          ...
          return '<div style="display:flex;align-items:center;gap:8px;margin-top:3px;">'
            ...
            + (_fr > 0 ? '<div title="🎁 Bonus: ' + _fr + '/' + (ws.freeGranted || 0) + '" style="width:' + _fp + '%;height:100%;background:linear-gradient(90deg,' + THEME.PURPLE_DARK + ',' + THEME.PURPLE + ');"></div>' : '')
            + (billingAvail > 0 ? '<div title="💰 Monthly: ' + billingAvail + '/' + (ws.limit || 0) + '" style="width:' + _bp + '%;height:100%;background:linear-gradient(90deg,' + THEME.GREEN + ',' + THEME.GREEN_LIGHT + ');"></div>' : '')
            + (rollover > 0 ? '<div title="🔄 Rollover: ' + rollover + '/' + (ws.rolloverLimit || 0) + '" style="width:' + _rp + '%;height:100%;background:linear-gradient(90deg,#6b7280,#9ca3af);"></div>' : '')
            + (dailyFree > 0 ? '<div title="📅 Free: ' + dailyFree + '/' + (ws.dailyLimit || 0) + '" style="width:' + _dp + '%;height:100%;background:linear-gradient(90deg,#d97706,' + THEME.YELLOW + ');"></div>' : '')
            + '</div>'
            + '<span style="font-size:10px;white-space:nowrap;font-family:' + FONT.MONO + ';line-height:1;">'
            + (_fr > 0 ? '<span style="color:' + THEME.PURPLE + ';" title="🎁 Bonus = promotional credits remaining">🎁' + _fr + '</span> ' : '')
            + '<span style="color:' + THEME.GREEN_LIGHT + ';" title="💰 Monthly = credits remaining in billing period">💰' + billingAvail + '</span> '
            + '<span style="color:#9ca3af;" title="🔄 Rollover = unused credits carried from previous period">🔄' + rollover + '</span> '
            + '<span style="color:' + THEME.YELLOW + ';" title="📅 Free = free credits refreshed daily">📅' + dailyFree + '</span> '
            + '<span style="color:' + THEME.CYAN + ';font-weight:700;" title="⚡ Available = Total - rolloverUsed - dailyUsed - billingUsed - freeUsed">⚡' + _availTotal + '</span>'
  ...
  function updateCreditDisplay() {
    ...
    if (nowEl && perWs.length > 0) {
      ...
      if (currentWs) {
        ...
        // v7.13.0: Use cAvailable (from calcAvailableCredits with freeUsed) instead of inline sum
        var _nowTotal = cAvailable;
    ...
    return fetch(url, {
      ...
      if ((resp.status === 401 || resp.status === 403) && headers['Authorization']) {
        ...
        if (cookieToken && cookieToken !== getBearerTokenFromStorage()) {
          saveBearerTokenToStorage(cookieToken);
          logInfo(fn, '🔄 Auto-recovered bearer token from session cookie (401/403 → retry possible)');
        } else {
  ...
  // ============================================
  // v7.10.3: Auto-detect current workspace.
  // Tier 1: POST mark-viewed API → workspace_id → wsById O(1) lookup (restored from v7.9.20)
  // Tier 2: Project Dialog XPath detection (fallback)
  ...
  function autoDetectCurrentWorkspace(bearerToken, correlationId) {
    ...
    if (window.__wsCurrentName) {
      ...
      for (var g = 0; g < perWs.length; g++) {
        if (perWs[g].fullName === window.__wsCurrentName || perWs[g].name === window.__wsCurrentName) {
          matched = perWs[g];
          break;
        }
      }
      if (matched) {
        logEntry(fn, '✅ GUARD — workspace already set authoritatively: "' + window.__wsCurrentName + '" (skipping detection)');
        updateCreditDisplay();
        return Promise.resolve();
      }
      // Name doesn't match any workspace — fall through to detection
      logWarn(fn, 'GUARD — __wsCurrentName="' + window.__wsCurrentName + '" not found in list, falling through to Tier 1');
    }

    // ---- Tier 1: POST mark-viewed API ----
    ...
    // v7.11.1: Only require projectId — cookie auth (credentials:'include') works without bearer token
    if (projectId) {
      var markViewedUrl = CREDIT_CFG.API_BASE + '/projects/' + projectId + '/mark-viewed';
      logEntry(fn, 'Tier 1 — POST mark-viewed (projectId=' + projectId + ', auth=' + (bearerToken ? 'bearer' : 'cookie') + ') [' + (correlationId || '') + ']');
      ...
      return fetch(markViewedUrl, { method: 'POST', headers: headers, credentials: 'include', body: '{}' })
        .then(function(resp) {
          logSub(fn, 'Tier 1 response: status=' + resp.status);
          if (!resp.ok) throw new Error('HTTP ' + resp.status);
          return resp.text().then(function(bodyText) {
            bodyText = (bodyText || '').trim();
            logSub(fn, 'Tier 1 body: ' + (bodyText || '(empty)').substring(0, 200));
        ...
        .then(function(data) {
          if (data && data.workspace_id) {
            var ws = wsById[data.workspace_id];
            if (ws) {
              window.__wsCurrentName = ws.fullName || ws.name;
              logEntry(fn, '✅ Tier 1 matched: workspace_id=' + data.workspace_id + ' → "' + window.__wsCurrentName + '"');
              updateCreditDisplay();
              return; // Done
            }
            logWarn(fn, 'Tier 1: workspace_id=' + data.workspace_id + ' not found in wsById — falling to Tier 2');
          } else {
            logWarn(fn, 'Tier 1: mark-viewed returned no workspace_id — falling to Tier 2');
          }
          return detectWorkspaceViaProjectDialogCombo(fn, perWs, wsById);
        })
        .catch(function(err) {
          logWarn(fn, 'Tier 1 failed: ' + err.message + ' — falling to Tier 2');
          return detectWorkspaceViaProjectDialogCombo(fn, perWs, wsById);
        });
    }

    // No projectId — skip Tier 1, go direct to Tier 2
    logWarn(fn, 'Tier 1 skipped (no projectId in URL) — going to Tier 2');
  ...
  function detectWorkspaceViaProjectDialogCombo(callerFn, perWs, wsById) {
    ...
    return new Promise(function(resolve) {
      ...
      var pollTimer = setInterval(function() {
        ...
        // v7.10.2: Use findAllByXPath — the XPath may match multiple elements.
        // Iterate all matches and validate each against known workspaces.
        var allNodes = findAllByXPath(workspaceXPath);
        if (allNodes.length > 0) {
          clearInterval(pollTimer);

          // Log all node texts for diagnostics
          for (var ni = 0; ni < allNodes.length; ni++) {
            logSub(fn, 'Node[' + ni + ']: "' + (allNodes[ni].textContent || '').trim() + '"');
          }
          ...
          if (matched) {
            ...
            logEntry(fn, '✅ Workspace detected from project dialog: "' + matchedRawName + '" → ' + window.__wsCurrentName + ' (id=' + matched.id + ') [checked ' + allNodes.length + ' nodes]');
          } else {
            logWarn(fn, 'No node matched a known workspace (checked ' + allNodes.length + ' nodes, ' + perWs.length + ' workspaces)');
            if (!window.__wsCurrentName) {
              window.__wsCurrentName = perWs[0].fullName || perWs[0].name;
              logSub(fn, 'Defaulted to first workspace: ' + window.__wsCurrentName);
            }
        ...
        if (elapsed >= dialogWaitMs) {
          clearInterval(pollTimer);
          logWarn(fn, 'WorkspaceNameXPath not found after ' + dialogWaitMs + 'ms — trying CSS selector fallback (S-012)');

          // S-012: CSS selector fallback for workspace name
          var cssFallbackNodes = findComboWorkspaceNameViaCss(fn, perWs);
          if (cssFallbackNodes.matched) {
            window.__wsCurrentName = cssFallbackNodes.matched.fullName || cssFallbackNodes.matched.name;
            logEntry(fn, '⚠️ Workspace detected via CSS fallback: "' + cssFallbackNodes.rawName + '" → ' + window.__wsCurrentName + ' (XPath may be stale — consider updating WorkspaceNameXPath in config.ini)');
            closeComboProjectDialogSafe(btn, projectBtnXPath);
            updateCreditDisplay();
            resolve();
            return;
          ...
          logWarn(fn, 'CSS fallback also failed — defaulting');
  ...
  // S-012: CSS selector fallback for workspace name detection (combo.js version)
  var COMBO_WS_CSS_SELECTORS = [
  ...
  function findComboWorkspaceNameViaCss(fn, perWs) {
    var result = { matched: null, rawName: '' };

    for (var si = 0; si < COMBO_WS_CSS_SELECTORS.length; si++) {
      var sel = COMBO_WS_CSS_SELECTORS[si];
      try {
        var els = document.querySelectorAll(sel);
        logSub(fn, 'CSS fallback [' + (si + 1) + '/' + COMBO_WS_CSS_SELECTORS.length + ']: "' + sel + '" → ' + els.length + ' element(s)');
        ...
        for (var ei = 0; ei < els.length; ei++) {
          ...
          for (var wi = 0; wi < perWs.length; wi++) {
            if (perWs[wi].fullName === text || perWs[wi].name === text) {
              logSub(fn, 'CSS fallback ✅ MATCH: selector="' + sel + '", text="' + text + '" → ' + perWs[wi].fullName);
              result.matched = perWs[wi];
              result.rawName = text;
              return result;
            }
            if (perWs[wi].fullName && perWs[wi].fullName.toLowerCase().indexOf(text.toLowerCase()) !== -1) {
              logSub(fn, 'CSS fallback ✅ PARTIAL MATCH: selector="' + sel + '", text="' + text + '" → ' + perWs[wi].fullName);
              result.matched = perWs[wi];
              result.rawName = text;
              return result;
            }
        ...
        logSub(fn, 'CSS fallback [' + (si + 1) + '/' + COMBO_WS_CSS_SELECTORS.length + ']: "' + sel + '" → ERROR: ' + e.message);
      }
    }

    logSub(fn, 'CSS fallback: no selectors matched a known workspace (' + COMBO_WS_CSS_SELECTORS.length + ' selectors tried, ' + perWs.length + ' workspaces)');
    return result;
  }
  ...
  function createControllerUI() {
    ...
    function positionController(position) {
      enableFloating();
      var margin = 20;
      ...
      logEntry('positionController', 'Moved ComboSwitch to ' + position);
    }
    ...
    function buildBody() {
      ...
      upBtn.title = 'Switch to previous project (Ctrl+Left)';
      ...
      downBtn.title = 'Switch to next project (Ctrl+Right)';
      ...
      hint.textContent = 'Ctrl+Left/Right (Combo) | Ctrl+Up/Down (Move) | Ctrl+Alt+S/M';
      ...
      focusCurrentBtn.onclick = function(e) {
        ...
        var token = window.__comboResolvedToken || resolveToken() || '';
      ...
      const tokenCookieBtn = document.createElement('button');
      tokenCookieBtn.textContent = '🍪 From Cookie';
      tokenCookieBtn.title = 'Read bearer token from lovable-session-id.id cookie, save, and refresh data';
      tokenCookieBtn.style.cssText = 'flex:1;padding:4px 8px;border:none;border-radius:4px;background:#b45309;color:#fef3c7;font-size:11px;cursor:pointer;font-family:' + FONT.BODY + ';';
      tokenCookieBtn.onclick = function(e) {
        e.preventDefault(); e.stopPropagation();
        retrieveTokenFromCookie('combo');
      };

      tokenBtnRow.appendChild(tokenSaveBtn);
      tokenBtnRow.appendChild(tokenPasteBtn);
      tokenBtnRow.appendChild(tokenCookieBtn);
    ...
    document.addEventListener('keydown', function(e) {
      ...
      // v7.9.33: Ctrl+Up/Down (no Alt) for force move — check BEFORE Ctrl+Alt guard
      const isCtrlOnly = e.ctrlKey && !e.altKey && !e.shiftKey;

      // v7.9.42: Ctrl+1 → bottom-left, Ctrl+3 → bottom-right
      if (isCtrlOnly && e.key === '1') {
        e.preventDefault();
        positionController('bottom-left');
        return;
      }
      if (isCtrlOnly && e.key === '3') {
        e.preventDefault();
        positionController('bottom-right');
        return;
      }

      if (isCtrlOnly && e.key === 'ArrowUp') {
        e.preventDefault();
        logEntry('keydown', 'Ctrl+Up → Force Move UP via API');
        moveToAdjacentWorkspaceCombo('up');
        return;
      }
      if (isCtrlOnly && e.key === 'ArrowDown') {
        e.preventDefault();
        logEntry('keydown', 'Ctrl+Down → Force Move DOWN via API');
      ...
      // v7.9.33: Combo uses Ctrl+Alt+Left/Right (was Up/Down)
      const isLeftArrow = e.key === 'ArrowLeft';
      if (isLeftArrow) {
        e.preventDefault();
        logEntry('keydown', 'Ctrl+Alt+Left pressed on settings page -> ComboSwitch UP');
        window.__comboSwitch('up');
        return;
      }
      ...
      if (isRightArrow) {
        e.preventDefault();
        logEntry('keydown', 'Ctrl+Alt+Right pressed on settings page -> ComboSwitch DOWN');
        window.__comboSwitch('down');
        return;
      }
    });
  ...
  if (isAlreadyEmbedded()) {
    ...
    // left the marker but failed before defining __comboSwitch.
    ...
    // Marker exists but globals missing — previous injection crashed. Clean up and re-init.
    logInfo('init', 'Stale marker found (globals missing) — removing marker and re-initializing');
    var staleMarker = document.getElementById(ID.SCRIPT_MARKER);
    if (staleMarker) staleMarker.remove();
    var staleContainer = document.getElementById(ID.CONTAINER);
  ...
  if (isOnSettingsPage()) {
    ...
    logWarn('init', 'Not on settings page — controller installed but combo NOT auto-run. Navigate to settings and use Ctrl+Alt+Left/Right.');
