...
| 2 | **Coding guidelines in 3 places** | `spec/coding-guidelines/`, `spec/06-coding-guidelines/`, `spec/05-chrome-extension/coding-guidelines/` |
...
| 7 | **Category confusion** | Chrome extension specs split across `05-chrome-extension/`, `04-chrome-project-scripts/`, `05-chrome-extension/testing/` |
...
```
spec/
├── 00-overview/                    # Master docs, README, architecture
...
│   └── 11-folder-policy.md
│
├── 02-data-and-api/                # Data schemas, API responses, DB specs
...
│
├── 01-app-issues/                      # All app issues (rename from 01-app-issues)
│   └── (all existing contents unchanged)
│
├── 03-tasks/                       # Roadmap, task breakdowns (from 03-tasks)
│   └── (all existing contents unchanged)
│
├── 04-macro-controller/            # Everything macro controller
│   ├── credit-system.md            (from 04-macro-controller/credit-system.md)
│   ├── workspace-management.md     (from 04-macro-controller/workspace-management.md)
│   ├── ui-controllers.md           (from 04-macro-controller/ui-controllers.md)
│   ├── ui-overhaul.md              (from 04-macro-controller/ui-overhaul.md)
│   ├── migration-v2.md             (from 04-macro-controller/migration-v2.md)
│   ├── workspace-name/             (from 04-macro-controller/workspace-name/)
│   ├── js-to-ts-migration/         (from 04-macro-controller/js-to-ts-migration/)
│   └── ts-migration-v2/            (from 04-macro-controller/ts-migration-v2/)
│
├── 05-chrome-extension/            # All chrome extension specs (merge 05-chrome-extension + imported)
│   └── (all existing 05-chrome-extension/ contents)
│   └── testing/                    (from 05-chrome-extension/testing/)
│
├── 06-coding-guidelines/           # Single source — merge all 3 locations
│   ├── 01-code-quality.md          (from spec/coding-guidelines/)
│   ├── typescript-standards/       (from 03-imported-spec/04 + 06-coding-guidelines/05)
│   ├── golang-standards/           (from 03-imported-spec/05 + 06-coding-guidelines/06)
│   ├── php-standards/              (from 03-imported-spec/06 + 06-coding-guidelines/07)
│   ├── coding-guidelines/          (from 03-imported-spec/03 + 06-coding-guidelines/04)
│   └── engineering-standards.md    (from 05-engineering-standards.md)
│
├── 07-devtools-and-injection/      # DevTools, injection, SDK
...
│
├── 08-features/                    # Feature specs (pstore, automation, sync)
...
│   └── storage-ui-redesign.md      (already in 05-chrome-extension/55)
│
├── 09-imported/                    # External/imported specs (error mgmt, WP, PS, etc.)
...
│
├── 10-prompts/                     # Prompt samples and structure
...
│
└── 03-tasks/next-feature.md         (standalone — move into 03-tasks/)
```
...
### Task 1 — Create `spec/00-overview/` and move overview docs
- Create `spec/00-overview/`
- Move: `README.md`, `00-master-overview.md`, `03-architecture.md`, `10-version-history-summary.md`, `11-folder-policy.md`

### Task 2 — Create `spec/02-data-and-api/` and consolidate data specs
- Create `spec/02-data-and-api/`
- Move `01-api-response/` → `spec/02-data-and-api/api-response/`
- Move `04-data-schema.md`, `17-data-models.md`, `23-json-schema-authoring-guide.md`, `24-marco-json-schema-guide.md`
- Move `04-db-join-specs/` contents into `spec/02-data-and-api/db-join-specs/`
- Delete empty `04-db-join-specs/`

### Task 3 — Rename `01-app-issues/` → `01-app-issues/`
- Rename folder

### Task 4 — Rename `03-tasks/` → `03-tasks/` and absorb `03-tasks/next-feature.md`
- Rename folder
- Move `03-tasks/next-feature.md` into `spec/03-tasks/`

### Task 5 — Create `spec/04-macro-controller/` and consolidate macro specs
- Create `spec/04-macro-controller/`
...
- Move standalone files: `04-macro-controller/credit-system.md`, `04-macro-controller/workspace-management.md`, `04-macro-controller/ui-controllers.md`, `04-macro-controller/ui-overhaul.md`, `04-macro-controller/migration-v2.md`
...
### Task 6 — Rename `05-chrome-extension/` → `05-chrome-extension/` and merge imported chrome specs
- Rename folder
- Move `05-chrome-extension/testing/` contents into `spec/05-chrome-extension/testing/`
- Merge `05-chrome-extension/coding-guidelines/` into the unified guidelines folder (Task 7)

### Task 7 — Create `spec/06-coding-guidelines/` and merge all 3 guideline locations
- Create `spec/06-coding-guidelines/`
- Move from `spec/coding-guidelines/`: `01-code-quality-improvement.md`
- Move from `spec/06-coding-guidelines/`: all subfolders (dedup against `03-imported-spec`)
- Move from `spec/03-imported-spec/03-coding-guidelines/` through `06-php-standards/`
- Move `05-engineering-standards.md`
- Delete empty `coding-guidelines/`, `06-coding-guidelines/`

### Task 8 — Create `spec/07-devtools-and-injection/`
...
### Task 9 — Create `spec/08-features/`
- Move: `20-pstore-marketplace.md`, `21-advanced-automation.md`, `22-cross-project-sync.md`

### Task 10 — Create `spec/09-imported/` and move remaining imported specs
...
### Task 11 — Rename `14-prompt-samples/` → `spec/10-prompts/` and absorb prompt docs
- Move `14-prompt-samples/` → `spec/10-prompts/`
...
- Search for old paths (`spec/05-chrome-extension/`, `spec/01-app-issues/`, etc.) across all `.md` files
