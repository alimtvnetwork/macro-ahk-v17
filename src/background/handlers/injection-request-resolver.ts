...
export async function resolveInjectionRequestScripts(
    ...
    if (mismatched > 0) {
        console.warn("[injection:resolve] ⚠ %d/%d scripts failed type check (missing 'id', 'code', or 'order'). These scripts will be SKIPPED silently unless fixed.", mismatched, scripts.length);
...
async function resolveProjectEntryScripts(
    ...
    // See: spec/01-app-issues/check-button/11-popup-injection-missing-guard.md
