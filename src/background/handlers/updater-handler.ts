...
export function handleCreateUpdater(data: {
    ...
    db.run(
        ...
        [
            data.name,
            data.scriptUrl,
            data.versionInfoUrl ?? null,
            data.instructionUrl ?? null,
            data.changelogUrl ?? null,
            data.isGit ? 1 : 0,
...
export function ensureUpdaterCategory(name: string): number {
    ...
    const existing = db.exec("SELECT Id FROM UpdaterCategory WHERE Name = ?", [name]);
    if (existing.length > 0 && existing[0].values.length > 0) {
        return existing[0].values[0][0] as number;
    }

    db.run("INSERT INTO UpdaterCategory (Name, CreatedAt) VALUES (?, datetime('now'))", [name]);
    const result = db.exec("SELECT last_insert_rowid() AS Id");
    dbManager?.markDirty();
