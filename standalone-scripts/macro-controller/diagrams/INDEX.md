...
Full injection pipeline from "Run Scripts" click through 6 stages with detailed annotations: Stage 0a validates built-in scripts with self-healing and per-script logging; Stage 0b resolves dependencies via topological sort with missing-dependency error logging; Stage 1 resolves scripts to executable form (unresolvable scripts are hard errors, not silent skips); Stage 2 parallel bootstrap (namespace root in MAIN world, relay install, token seeding); Stage 3 wraps each script in IIFE isolation with SDK preamble and try-catch error boundaries — batch concatenation for no-CSS scripts, sequential CSS-first injection otherwise; Stage 4 CSP-aware execution with 3-tier fallback (MAIN blob → userScripts API → ISOLATED last resort with DEGRADED health); Stage 5 populates data namespaces (Settings + Projects) after scripts execute; post-pipeline mirrors all logs to tab DevTools + SQLite, checks performance budget, verifies 6 globals, and shows success/warning/error toast. Full clarification spec: `spec/05-design-diagram/mermaid-design-diagram-spec/01-diagram-spec/injection-pipeline-workflow-clarification-and-correction.md`.
