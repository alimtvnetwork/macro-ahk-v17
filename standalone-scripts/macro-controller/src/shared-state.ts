...
// Flush validation warnings to activity log (deferred until log module is available)
const _configWarnings = drainValidationWarnings();
if (_configWarnings.length > 0) {
  // Log warnings after module initialization completes (setTimeout ensures log() is available)
  setTimeout(function() {
