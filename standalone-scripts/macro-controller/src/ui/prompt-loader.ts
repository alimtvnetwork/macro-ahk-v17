...
export const DEFAULT_PROMPTS: PromptEntry[] = [
  { name: 'Start Prompt', text: 'Write a readme.txt text file with 3 words with no context at all "let\'s start now {date:dd-MMM-YYYY} {time:12 hr clock format exact time now for malaysia}"', slug: 'start-prompt', id: 'default-start' },
  { name: 'Start Prompt v2', text: 'Write a readme.txt text file with 3 words with no context at all "let\'s start now {date:dd-MMM-YYYY} {time:12 hr clock format exact time now for malaysia}"\n\nUh, try to write a file to the Git system. I\'m not sure what you are doing. I\'m asking you to write the text file in the read.txt file, readme.txt file with the date and time, and you are not doing it. You are not doing a Git, uh, update. Are you stupid?', slug: 'start-prompt-v2', id: 'default-start-v2' },
  ...
  { name: 'Unified AI Prompt v4', text: 'Read and synthesize existing repository context from the memory folder and the full specification set. Follow the Required Execution Order: scan repo, read memory, read specs, reconstruct context, produce reliability report, propose corrections, update memory, update plan, ask user which task to implement next.', slug: 'unified-ai-prompt-v4', id: 'default-unified' },
  { name: 'Issues Tracking', text: 'Do not implement any code changes. Update specifications and documentation only. Enforce a strict workflow so the same mistakes do not repeat, and ensure every fix is recorded in a standardized issue write-up file and reflected in memory.', slug: 'issues-tracking', id: 'default-issues' },
  ...
  { name: 'Audit Spec v1', text: 'Perform a comprehensive audit of every specification file. Score each spec on Completeness, Consistency, Implementation Alignment, Clarity, Maintainability, and Test Coverage (1-10 scale). Produce a scorecard, detailed findings for specs below 8.0, cross-spec dependency map, and priority fix list. Write report to .memory/audit/spec-audit-report.md.', slug: 'audit-spec-v1', id: 'default-audit' },
